...
from file_rag.knowledge_base.kb_summary.base import KBSummaryService
from file_rag.knowledge_base.kb_summary.summary_chunk import SummaryAdapter
from file_rag.knowledge_base.model.kb_document_model import DocumentWithVSId
from file_rag.knowledge_base.utils import list_files_from_folder
from file_rag.server.utils import BaseResponse, wrap_done, get_default_embedding
        ...
        try:
            ...
            if not kb.exists() and not allow_empty_kb:
                ...
                if not ok:
                    ...
                    reduce_llm = Ollama(model="qwen:latest")
                    # llm = get_ChatOpenAI(
    ...
    def output():
        try:
            ...
            if not kb.exists() and not allow_empty_kb:
                ...
                kb_summary = KBSummaryService(knowledge_base_name, embed_model)
                kb_summary.create_kb_summary()
                llm = Ollama(model='qwen:latest',temperature=temperature)
                reduce_llm = Ollama(model='qwen:latest',temperature=temperature)
...
if __name__ == "__main__":
   summary_file_to_vector_store(knowledge_base_name = "samples",
file_name = "./tests/samples/大模型技术栈-实战与应用.md",model_name='qwen:latest', temperature='0.5')