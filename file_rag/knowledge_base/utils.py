...
from file_rag.server.utils import run_in_process_pool, run_in_thread_pool
...
class KnowledgeFile:
    ...
    def file2docs(self, refresh: bool = False):
        if self.docs is None or refresh:
            # print(f'ffffffffffffffffffffff{self.document_loader_name}')
...
def format_reference(kb_name: str, docs: List[Dict], api_base_url: str="") -> List[Dict]:
    ...
    kb_path = Settings.basic_settings.KB_ROOT_PATH
    source_documents = []
    path =''
    f_name = []
    urls  = []
    # print(f'kb_name-------{kb_name}')
    # print(F'format_reference---{format_reference}')
    for inum, doc in enumerate(docs):
        ...
        ref = f"""来自 [{inum + 1}] [{filename}]({url}) \n\n{page_content}\n\n"""
        # ref = f"""{url}"""
        urls.append(url)
        source_documents.append(ref)
    # if f_name[0].split('.')[1] == 'html':
    #     path = f'./data/knowledge_base/{kb_name}/content/{f_name[0]}'
    #     # path = urls[0]
    # else:
    if kb_name is not None:
        path = f'{kb_path}\\{kb_name}\\content\\{f_name[0]}'
    # 涉及到格式转换，进行预览.全部转换为pdf
    # if f_name[0].split('.')[1] == 'html':
    #    import pdfkit
...
def format_reference1(kb_name: str, docs: List[Dict], api_base_url: str="") -> List[Dict]:
    '''
    将知识库检索结果格式化为参考文档的格式
    '''
    from file_rag.server.utils import api_address
