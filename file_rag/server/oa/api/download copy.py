...
def _Get_docFile(token, docId, subject, kb_name, businessName):
    ...
    if businessName == '发文':
        ...
        doc_info = f'{api}/doc/receival/getReceivalById?docId={docId}&_t=1725863919760'
    # 正文
    main_url = f'{api}/attachment/listEgovAttsByDocIdAndTypes?docId={docId}&type%5B%5D=main_doc&type%5B%5D=main_trace&type%5B%5D=main_ofd&type%5B%5D=main_pdf&type%5B%5D=main_copy&containFile=false&moduleId=&_t=1724135953238'
    # 附件
    attach_url = f'{api}/attachment/listEgovAttsByDocIdAndTypes?docId={docId}&type%5B%5D=attach&containFile=false'
    # 办理单
    dealForm_url = f'{api}/attachment/listEgovAttByDocId?docId={docId}&type=dealForm'
    ...
    for item in json_data:
        ...
        if similarity > max_similarity:
            ...
            most_similar_businessSubject = most_similar_item.get(
                'businessSubject', '')
            most_similar_businessName = most_similar_item.get('businessName','')
...
if __name__ == "__main__":
    find_subjectId('64230a47-1e33-43db-b51d-da7cef559a5d', '弱口令')
