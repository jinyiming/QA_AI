...
from file_rag.server.api_server.api_schemas import OpenAIChatInput
from file_rag.server.chat.chat import chat
from file_rag.server.chat.kb_chat import kb_chat
# from file_rag.server.chat.feedback import chat_feedback
from file_rag.server.chat.file_chat import file_chat
# from file_rag.server.chat.login import _userLogin_sso, LoginRequest
from file_rag.server.db.repository import add_message_to_db
...
from ..chat.login import _userLogin_sso
...
# @chat_router.post("/_userLogin_sso", summary="登录")
# async def login(request: LoginRequest):
#     return await _userLogin_sso(request)
...
async def chat_completions(
    ...
    if body.tool_choice:
        ...
        if tool_input := extra.get("tool_input"):
            ...
            print(f'tool_result------ {body.tool_choice}')
            if body.tool_choice['function']['name'] == 'search_oa':
                prompt_template = PromptTemplate.from_template(
                    get_prompt_template("OA", "default"), template_format="jinja2"
                )
                body.messages[-1]["content"] = prompt_template.format(
                context=tool_result, question=body.messages[-1]["content"].split('|||')[0]  
                )
                print(f'body.messages[-1]["content"] ---- {body.messages[-1]["content"]}')
            else:
                prompt_template = PromptTemplate.from_template(
