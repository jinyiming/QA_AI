...
from file_rag.server.pydantic_v1 import Field
from file_rag.server.utils import get_tool_config

from .tools_registry import BaseToolOutput, regist_tool

# READ_ONLY_PROMPT_TEMPLATE = """You are a SQL expert. The database is currently in read-only mode. 
...
# Question: {query}
# """
READ_ONLY_PROMPT_TEMPLATE = """你是一个SQL专家，当前数据库处于只读状态。
根据问生成合适的sql语句，记住只生成可执行的sql语句，禁止使用特殊符号```sql```进行修饰。
如果 SQL 可以正常执行，则返回 Answer：'SQL can be executed normal'。
如果 SQL 无法正常执行，则返回 Answer： 'SQL cannot be executed normal'。
使用以下格式：
回答: 最终答案（只输出可执行的sql语句，不加任何修饰）
问题: {query}
"""
...
def query_database(query: str, config: dict):
    ...
    db = SQLDatabase.from_uri(sqlalchemy_connect_str,schema='XYCS')
    
    from file_rag.server.utils import get_ChatOpenAI
