...
from file_rag.settings import Settings, XF_MODELS_TYPES
from file_rag.server.pydantic_v2 import BaseModel, Field
...
def get_ChatOpenAI(
    ...
    for k in list(params):
        ...
        # model = Ollama(model='qwen2:latest',base_url='http://localhost:11434')
    try:
        if local_wrap:
            params.update(
...
#     from file_rag.server.localai_embeddings import (
#         LocalAIEmbeddings,
#     )
...
def get_Embeddings(
    ...
    return OllamaEmbeddings(model=embed_model.split('/')[1], base_url='http://localhost:11434')
...
def get_OpenAIClient(
    ...
    """
    print(model_name,platform_name)
...
def update_search_local_knowledgebase_tool():
    ...
    from file_rag.server.agent.tools_factory import tools_registry
    from file_rag.server.db.repository.knowledge_base_repository import list_kbs_from_db
...
def get_tool(name: str = None) -> Union[BaseTool, Dict[str, BaseTool]]:
    ...
    from file_rag.server.agent import tools_factory
