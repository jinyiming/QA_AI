...
from file_rag.server.agent.agent_factory.agents_registry import agents_registry
from file_rag.server.api_server.api_schemas import OpenAIChatOutput
from file_rag.server.callback_handler.agent_callback_handler import (
    AgentExecutorAsyncIteratorCallbackHandler,
    AgentStatus,
)
from file_rag.server.chat.utils import History
from file_rag.server.memory.conversation_db_buffer_memory import (
...
def create_models_from_config(configs, callbacks, stream, max_tokens):
    ...
    models = {}
    prompts = {}
    for llm_type, params in configs.items():
        llm_model = params.get("model", "").strip() or get_default_llm()
        ...
        models[llm_type] = model_instance
        prompt_name = params.get("prompt_name", "default")
        prompt_template = get_prompt_template(type=llm_type, name=prompt_name)
        prompts[llm_type] = prompt_template
...
async def chat(
    ...
    async def chat_iterator() -> AsyncIterable[OpenAIChatOutput]:
        try:
            ...
            llm_model = chat_model_config["llm_model"].get("model", "").strip() or get_default_llm()
            ...
            async for chunk in callback.aiter():
                ...
                ret = OpenAIChatOutput(
                    ...
                    model=models["llm_model"].llm_model,
                    status=data["status"],
