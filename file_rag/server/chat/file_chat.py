...
from file_rag.knowledge_base.kb_cache.faiss_cache import memo_faiss_pool,KBFaissPool
from file_rag.knowledge_base.utils import KnowledgeFile
...
def _parse_files_in_thread(
    ...
    def parse_file(file: UploadFile) -> dict:
        ...
        try:
            ...
            # 为每个文档添加标题
            for doc in docs:
                doc.metadata["title"] = os.path.splitext(filename)[0]  # 使用文件名（不包括扩展名）作为标题
...
请先上传环评报告等文件以启用编制行为监督检查、项目风险评测分析等功能！""",
    ...
    async def knowledge_base_chat_iterator() -> AsyncIterable[str]:
        try:
            ...
            #     callbacks=callbacks,
            # )
            model = Ollama(model='qwen:latest', temperature=temperature)
            ...
            for inum, doc in enumerate(docs):
                ...
                title = doc.metadata.get("title", "未知标题")  # 获取文档标题
                text = f"""出处 [{inum + 1}] [{filename}] - {title}\n\n{doc.page_content}\n\n"""
