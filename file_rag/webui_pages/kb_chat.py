...
from file_rag.knowledge_base.utils import LOADER_DICT
from file_rag.server.utils import get_config_models, get_config_platforms, get_default_llm, api_address
from file_rag.webui_pages.dialogue.dialogue import (save_session, restore_session, rerun,
...
def kb_chat(api: ApiRequest):
    ...
    if prompt:
        ...
        try:
            ...
            #     with open(path, "rb") as f:
            #                 base64_pdf = base64.b64encode(f.read()).decode('utf-8')
            #     pdf_display = f'<iframe src="{path}";  height= "500"  width= "100%" ></iframe>'
            #     st.markdown(pdf_display, unsafe_allow_html=True)   
