...
import os
import uuid
from common.Ifasr_new import RequestApi
...
from file_rag.server.callback_handler.agent_callback_handler import AgentStatus
from file_rag.knowledge_base.model.kb_document_model import DocumentWithVSId
from file_rag.knowledge_base.utils import format_reference
from file_rag.server.utils import MsgType, get_config_models, get_config_platforms, get_default_llm
from file_rag.webui_pages.utils import *
recognizer = sr.Recognizer()
chat_box = ChatBox(assistant_avatar=get_img_base64(
...
def save_session(conv_name: str = None):
    ...
    chat_box.context_from_session(
        conv_name, exclude=["selected_page",
                            "prompt", "cur_conv_name", "upload_image"]
    )
...
def transcribe_audio(audio_data):
    """将音频数据转录为文本"""
    api = RequestApi(appid="ade84ce6",
                     secret_key="b6a5a24dc5841c698694bd1d63b44e53",
                     upload_file_path=audio_data)
    try:
        # 使用Google的语音识别服务
        data = api.get_result()
        ...
        if 'lattice2' in order_result:
            lattice2 = order_result['lattice2'][0]
            json_1best = lattice2['json_1best']
            for ws in json_1best['st']['rt'][0]['ws']:
                word = ws['cw'][0]['w']
                extracted_text += word 
...
def del_conv(name: str = None):
    ...
    if len(conv_names) == 1:
        ...
        sac.alert(
            "删除会话出错", f'无效的会话名称：会话{name}', color="error", closable=True
        )
    else:
...
# 删除无效的知识库
# def delete_kb(_api: ApiRequest,kb):
#     return _api.delete_knowledge_base(kb)
...
def dialogue_page(
    ...
    def llm_model_setting():
        ...
        temperature = cols[2].slider(
            "Temperature", 0.0, 1.0, key="temperature")
    ...
    with st.sidebar:
        ...
        with tab1:
            use_oa = st.checkbox(
                # delete_kb(api,'Kb_1962300fcc0a2000')
                "启用OA系统", help="请确保是否拥有OA账号", key="oa_agent"
            )
            ...
            if use_agent:
                ...
                selected_tool = st.selectbox(
                    "选择工具",
                    tool_names,
                    format_func=lambda x: tools.get(
                        x, {"title": "None"})["title"],
                    key="selected_tool",
                )
...
def transcribe_audio(audio_data):
    ...
    try:
        ...
        # parsed_data = json.dumps(data)
        order_result = data.get('content', {}).get('orderResult', '{}')
        if isinstance(order_result, str):
            try:
                order_result = json.loads(order_result)
            except json.JSONDecodeError:
                print("Invalid JSON in orderResult")
                return ""
        extracted_text = ""
        if 'lattice2' in order_result:
        # Get the first (and only) element in 'lattice2'
            lattice2 = order_result['lattice2'][0]
            for lat in order_result['lattice2']:
        # Extract the 'json_1best' part and parse it if it's a string
                json_1best = lat['json_1best']
                for ws in json_1best['st']['rt'][0]['ws']:
                    word = ws['cw'][0]['w']
                    extracted_text += word
        return extracted_text
...
def dialogue_page(
    ...
    # chat input
    with bottom():
        cols = st.columns([1, 0.25, 15,  1, 0.25, 3])
        if cols[0].button(":gear:", help="模型配置"):
            widget_keys = ["platform", "llm_model",
                           "temperature", "system_message"]
            chat_box.context_to_session(include=widget_keys)
            llm_model_setting()
        if cols[3].button(":wastebasket:", help="清空对话"):
            chat_box.reset_history()
            rerun()
        with cols[-1]:
            qa_audio_bytes = audio_recorder("", icon_size="2x")
            
            if qa_audio_bytes:
                # 将音频字节保存为临时文件
                with open("temp_audio.wav", "wb") as f:
                    f.write(qa_audio_bytes)
                transcription = transcribe_audio("temp_audio.wav")
                print(transcription)
                # 自动提交输入框
                prompt = cols[2].chat_input(transcription, key="prompt")
                prompt = transcription
                # if prompt:  # 如果有转录文本，自动提交

                #     chat_box.user_say(prompt)
            else:           
                prompt = cols[2].chat_input(chat_input_placeholder, key="prompt")
    # if qa_audio_bytes:
    #     # user_query, is_success = audio2text(qa_audio_bytes, from_language=st.session_state.language)
    #     # 存储成文件，翻译，删除文件
    #     temp_path = get_audio_path(event_id="temp", audio_md5=str2md5(qa_audio_bytes))
    #     save_audio(audio=qa_audio_bytes, f_path=temp_path)
    #     with open(temp_path, "rb") as qa_audio_f:
    #         user_query, is_success = audio2text(qa_audio_f, from_language=st.session_state.language)
    #     logger.info(f"删除临时语音文件：{temp_path}")
    #     os.remove(temp_path)

    #     st.session_state.qa_audio_key = random.randint(1000, 100000000)
    #     logger.info(f"is_success: {is_success}, user_query: {user_query}")
    #     if is_success:
    #         qa_status = True
    #     else:
    #         with answer_container:
    #             st.error("音频识别失败，请重新录音!")
    # # floating_audio_bar()
    ...
    if prompt:
        ...
        if is_vision_chat:  # multimodal chat
            chat_box.user_say(
                [Image(get_image_file_url(upload_image), width=100), Markdown(prompt)])
        else:
            chat_box.user_say(prompt)
        if files_upload:
            if files_upload["images"]:
                st.markdown(
                    f'<img src="data:image/jpeg;base64,{
                        files_upload["images"][0]}" width="300">',
                    unsafe_allow_html=True,
                )
            elif files_upload["videos"]:
                st.markdown(
                    f'<video width="400" height="300" controls><source src="data:video/mp4;base64,{
                        files_upload["videos"][0]}" type="video/mp4"></video>',
                    unsafe_allow_html=True,
                )
            elif files_upload["audios"]:
                st.markdown(
                    f'<audio controls><source src="data:audio/wav;base64,{
                        files_upload["audios"][0]}" type="audio/wav"></audio>',
                    unsafe_allow_html=True,
                )
        ...
        if is_vision_chat:  # multimodal chat
            content = [
                ...
                {"type": "image_url", "image_url": {
                    "url": get_image_file_url(upload_image)}}
            ]
            messages = [{"role": "user", "content": content}]
        elif tool_choice == 'search_oa':
            prompt = prompt + '|||' + st.session_state['oa_session'] + '|||' + st.session_state['oa_userid'] + '|||' + str(
                history) + '|||' + st.session_state['systemNo'] + '|||' + st.session_state['orgNo']
        ...
        if stream:
            ...
            try:
                ...
                chat_box.update_msg(
                    d.choices[0].message.content or "", streaming=False)
            except Exception as e:
                st.error(e.body)
