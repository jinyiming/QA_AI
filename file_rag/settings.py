...
from file_rag.pydantic_settings_file import *
...
class BasicSettings(BaseFileSettings):
    ...
    API_SERVER: dict = {"host": DEFAULT_BIND_HOST, "port": 7878, "public_host": "127.0.0.1", "public_port": 7878}
...
class ApiModelSettings(BaseFileSettings):
    ...
    DEFAULT_LLM_MODEL: str = "qwen2"
    """默认选用的 LLM 名称"""

    DEFAULT_EMBEDDING_MODEL: str = "quentinz/bge-large-zh-v1.5"
    """默认选用的 Embedding 名称"""
...
class ToolSettings(BaseFileSettings):
    ...
    text2sql: dict = {
        ...
        "sqlalchemy_connect_str": "dm+dmPython://XYCS:Xycs#2021@localhost",
        ...
        "table_comments": {
            ...
            "ums_user": "这是一个用户表，存储了用户的基本信息,user_no 用户唯一标识。",
            "ums_org": "组织结构表，存储单位以及部门信息，org_no 组织唯一标识。",
            "ums_user_org_relate": "用户与组织关联表。包括user_no 与 org_no。",
        },
    }
    '''
    text2sql使用建议
